...
void start(){
    Task * task = NULL;
    while(!terminate || centralizedTaskQueue->size()>0){
        task = NULL;
        int count =0;
        while(ready && task == NULL && (count < max_attempts)&& centralizedTaskQueue->size()>0){
...
int initScheduler(){
    ...
    centralizedTaskQueue->push_back(initialTask);
    int id =0;
    while(id < maxCores-1){
        cilk_spawn start();
        id++;
