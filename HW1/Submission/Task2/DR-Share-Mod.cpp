...
public:
    Task( int Zr,int Zc,
          int Xr,int Xc,
          int Yr,int Yc,
          int n ,std::vector<int>* stack,mutex* stack_lock) : Zr(Zr),Zc(Zc),Xr(Xr),Xc(Xc),Yr(Yr),Yc(Yc),n(n),stack(stack),stack_lock(stack_lock) {}
...
std::unordered_map<int,long> work;

int mm(int** Z, int** X, int** Y, int Zr,int Zc, int Xr,int Xc, int Yr,int Yc, int n);
int _sync(std::vector<int>* stack,mutex* stack_lock,int **Z, int **X, int **Y);
void second_half(int **Z, int **X, int **Y, int Zr, int Zc, int Xr, int Xc, int Yr, int Yc, int n,std::vector<int>* stack) ;
...
int spawn(int Zr,int Zc,
    ...
    int p1Id = processIds[getRandom()];
    int p2Id = processIds[getRandom()];
    int pId = (work[p1Id] < work[p2Id])?p1Id:p2Id;
    work[pId] += n*n*n;
    processQueue[pId]->push_back(task);
    return 1;
}
...
int initScheduler(){
    ...
    std::vector<int>* stack = new vector<int>();
    stack->push_back(0);
    stack->push_back(0);
    stack->push_back(0);
    stack->push_back(0);
