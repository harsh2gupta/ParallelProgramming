...
#include <chrono>
#include <cilk/cilk.h>
#define R 4096 // 2^12, therefore R = 12
...
void iter_IKJ_I(recMatrix X, recMatrix Y, recMatrix Z){
...
int main(){
    ...
    cout <<"par_rec_mm 32, "<< diff.count() << " ms" << Z[2][2]<<endl;
    reset(X, Y, Z);
    xb = {0,R,0,R};
    yb = {0,R,0,R};
    zb = {0,R,0,R};
    ...
    cout <<"iter_IKJ_I, "<< diff.count() << " ms, "<< Z[2][2]<<endl;
